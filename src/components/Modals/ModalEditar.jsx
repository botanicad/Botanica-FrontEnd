...
export default function ModalEditar({ onClose, onSubmit, planta }) {
  ...
  useEffect(() => {
    if (planta) {
      ...
      setPreview(planta.imagemBase64 ? `data:image/jpeg;base64,${planta.imagemBase64}` : null);
    }
  }, [planta]);
  ...
  const handleSubmit = () => {
    const dadosComImagem = { ...form };

    if (imagem) {
      const reader = new FileReader();
      reader.onloadend = () => {
        const base64String = reader.result.split(",")[1];
        dadosComImagem.imagemBase64 = base64String;
        onSubmit(dadosComImagem);
        onClose();
      };
          ...
          <label className="border-2 border-dashed border-gray-300 w-full md:w-32 h-32 flex flex-col justify-center items-center rounded-lg cursor-pointer shrink-0 shadow-sm bg-gray-50">
            {preview ? (
              ...
              <>
                ...
                <p className="text-center text-xs text-gray-500 font-medium">Alterar imagem</p>
              </>
            )}
